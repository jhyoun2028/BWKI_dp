...
- ❌ **Colab DistilBERT results still MISSING (2026-09-10)** — no `results/colab/`, no Drive export, nothing new on `main`; `metrics.json` holds only the two baseline variants. Nothing was merged (nothing to merge). `src/tune_threshold.py` picks DistilBERT up automatically as soon as `models/distilbert/` exists. **Notebook 03 must still be run on Colab against the CURRENT splits (4,408 / 513 / 617).**
...
- ⏭️ Next: more REAL German legit messages (formal tone: parcel, bank, doctor, authority) AND German rows in val so thresholds can be tuned honestly; Colab run of notebook 03 on the NEW splits; then Step 9 (error_analysis.md) and Step 10 (README).
