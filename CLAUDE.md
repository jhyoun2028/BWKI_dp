...
- ✅ **Best configuration is unchanged: baseline_tfidf_logreg at the DEFAULT boundaries (yellow ≥ 0.50, red ≥ 0.80) + trusted-link cap.** `eval_pipeline.py` now takes `--yellow-p` / `--red-p`. Best config: legit 10 green / 6 yellow / 2 red, phishing 2 / 11 / 73 → product false-alarm rate 0.444, legit→red 0.111, phishing→red 0.849. With the tuned 0.22 boundary: legit 4 / 12 / 2 → false-alarm rate **0.778** at identical phishing→red 0.849. Both experiments appended to `experiments.csv`; 58 tests pass.
...
- ⚠️ **DistilBERT is NOT usable locally yet:** `models/distilbert/` (~540 MB, Google Drive) is not in the repo, so `pipeline.py`, `eval_pipeline.py` and `tune_threshold.py` still run the baseline. Copy the Drive folder to `models/distilbert/` to switch the product over; nothing else needs changing (fallback logic already handles it). Threshold tuning could therefore NOT be extended to DistilBERT (a sweep needs per-row probabilities, `metrics.json` only stores the 15 misclassified rows).
...
- ⏭️ Next: copy `models/distilbert/` from Drive and re-run `eval_pipeline.py` + `tune_threshold.py` (then DistilBERT is included automatically); more REAL German legit messages (formal tone) AND German rows in val so thresholds can be tuned honestly; then Step 9 (error_analysis.md) and Step 10 (README).
