...
- 📱 **Subproject `android/` on branch `android-app` (2026-09-12)** — minimal Kotlin/Compose app, minSdk 29, targetSdk 34, no detection logic of its own: it calls the existing FastAPI. Screens: Eingabefeld + großer „Prüfen“-Knopf, farbige Ampelbox mit `reason_de`, Linkliste mit Stufe; Einstellungen für die Serveradresse (SharedPreferences, Standard leer, „Verbindung testen“ gegen `/health`). Funktionen: getippter Text → `POST /scan-text`; geteilter Text (`ACTION_SEND text/plain`) und geteiltes Bild (`ACTION_SEND image/*` → `POST /scan`, Feld `file`) werden sofort geprüft; Kachel in den Schnelleinstellungen (`TileService`, ab Android 14 via `PendingIntent`). Jede Anfrage trägt `ngrok-skip-browser-warning: true` (Retrofit `@Url` + OkHttp-Interceptor). Barrierearm: nichts unter 20 sp, Knöpfe ≥ 64 dp, kontraststarke Ampelfarben. Nur HTTPS erlaubt, Ausnahmen für Emulator/localhost in `network_security_config.xml`. **Keine Signierungsschlüssel im Repo**, `android/.gitignore` schließt `*.jks`/`*.keystore`/`local.properties` aus. Gradle-Wrapper 8.7 liegt bei. **Hier nicht kompilierbar** – kein Android SDK, `dl.google.com` gesperrt.
- 🛠️ **Android-Build-Fehler behoben (2026-09-12):** der erste echte Gradle-Build brach mit 4 Kotlin-Fehlern in `ScanViewModel.kt` ab (`Unresolved reference: Companion` für `okhttp3.MediaType.Companion.toMediaTypeOrNull` / `okhttp3.RequestBody.Companion.toRequestBody`). **Ursache belegt über Maven Central:** Retrofit 2.11.0 deklariert `com.squareup.okhttp3:okhttp:3.14.9` – die Java-Fassung ohne Kotlin-Erweiterungen; im JAR 3.14.9 existieren die Klassen `MediaType$Companion`/`RequestBody$Companion` nicht, im JAR 4.12.0 schon (per `javap` geprüft, Kotlin-Namen `toMediaTypeOrNull`/`toRequestBody` in den Metadaten). **Fix:** `okhttp-bom:4.12.0` + `com.squareup.okhttp3:okhttp` in `app/build.gradle.kts` (4.12.0 verlangt API 21, minSdk 29 passt; zieht Okio 3.6.0). Der Anwendungscode bleibt unverändert. Neue Gradle-Aufgabe `:app:okhttpVersion` druckt die tatsächlich aufgelöste Version. Nur diese eine Datei nutzte die Erweiterungen (per grep über alle `.kt` geprüft). Build weiterhin nicht hier verifizierbar.
